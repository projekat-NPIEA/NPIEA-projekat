...
def load_knapsack_data(filepath):
    with open(filepath, 'r') as f:
        lines = f.readlines()
    capacity = int(lines[0].split(':')[1].strip())
    items = []
    for line in lines[1:]:
...
def knapsack_cost(chromosome):
    ...
    if total_weight > capacity:
        return 0  # penalizuj rešenje ako je pretežak
    return -total_value  # jer rank_chromosomes minimizira!

if __name__ == "__main__":
    # Učitavanje podataka
    filename = "C:/Users/Administrator/Documents/II Godina/Evolutivni algoritmi/Teme za projekat/Tekstovi projekata/NPiEA - Projektni zadatak 02/data_knapsack01.txt"
    capacity, items = load_knapsack_data(filename)
    ...
    population = generate_inital_chromosomes(chromosome_length, 1, 0, population_size)

    for gen in range(generations):
        # 2. Evaluacija
        ranked = rank_chromosomes(knapsack_cost, population)
        population_sorted = ranked[0]
        scores = ranked[1]

        # 3. Elitizam
        parents = natural_selection(population_sorted, selection_size)

        # 4. Selekcija za ukrštanje
        pairs = roulette_selection(parents)
        ...
        # 6. Mutacija
        # Pretvaranje liste bitova u stringove jer mutation radi sa stringovima
        children_str = [''.join(str(bit) for bit in child) for child in children]

        # Poziv funkcije mutation (koja radi sa stringovima)
        mutated_children_str = mutation(children_str, mutation_rate)

        # Povratak u listu bitova (string -> lista int-ova)
        mutated_children = [[int(bit) for bit in child] for child in mutated_children_str]

        # 7. Elitizam + nova generacija
        population = elitis(list(population_sorted), mutated_children, elitism_rate, population_size)

    # Finalna evaluacija
    final_ranked = rank_chromosomes(knapsack_cost, population)
    best_solution = final_ranked[0][0]
    best_value = -knapsack_cost(best_solution)  # obrni nazad znak
    total_weight = sum(weight for gene, (weight, _) in zip(best_solution, items) if gene == 1)

    print("Najbolje rešenje:", best_solution)
    print("Ukupna vrednost:", best_value)
    print("Ukupna težina:", total_weight)
